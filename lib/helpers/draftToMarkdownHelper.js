...
module.exports = {
  ...
  processInlineStyles: (block, charsToInject) => {
    for (let index in block.inlineStyleRanges) {
      ...
      let textToBeStyled = block.text.substring(
        openingPosition,
        closingPosition
      );
      //only applying styled to trimmed text. required for markdown syntax
      let trimmedTextTobeStyled = textToBeStyled.trim();
      if( trimmedTextTobeStyled.length === 0 ){
        continue
      }
      if (textToBeStyled.length !== trimmedTextTobeStyled.length) {
        openingPosition = block.text.indexOf(trimmedTextTobeStyled);
        closingPosition = openingPosition + trimmedTextTobeStyled.length;
      }

      //TODO: find what this edge case is for
      !charsToInject[openingPosition] && (charsToInject[openingPosition] = '');

      if (block)
        charsToInject[openingPosition] =
          InlineMarkdownChars[inlineStyle.style] +
          charsToInject[openingPosition];
